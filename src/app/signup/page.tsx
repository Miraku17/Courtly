...
export default function SignUpPage() {
  ...
  const mutation = useMutation({
    ...
    onError: (error: any) => {
      const messages = error?.response?.data?.error || error?.response?.data?.message;
      ...
      <div className="relative hidden flex-1 flex-col justify-between p-12 lg:flex">
        ...
        >
          <p className="text-[0.9rem] font-medium text-text-muted/60">
            &copy; {new Date().getFullYear()} Courtify. Join the revolution.
          </p>
        ...
        >
          ...
          <div className="mb-6">
            ...
            <p className="text-[0.95rem] text-text-muted/70">
              Join Courtify and start booking courts today
            </p>
          </div>
